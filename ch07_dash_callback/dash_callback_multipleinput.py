...
app = dash.Dash(
    ...
    suppress_callback_exceptions=True # Dashでは起動時にコールバックで用いられているIdとレイアウトに存在するIDを確認し, 一致しない場合は例外を送出する仕様となっている。今回は、全てのIDがレイアウトに含まれているわけではないので、この例外が発生する。これを抑制するのがこの記述.
)
